...
const GET_USERS_FOR_PLAYER_CREATION = gql`
  query GetUsersForPlayerCreation {
    adminUsers {
      nodes {
        id
        name
...
const CreatePlayerForm: React.FC<CreatePlayerFormProps> = ({
  onSubmit,
  onCancel,
  isLoading = false
}) => {
  ...
  const { data: usersData, loading: usersLoading } = useQuery(GET_USERS_FOR_PLAYER_CREATION, {
    errorPolicy: 'all'
  });
